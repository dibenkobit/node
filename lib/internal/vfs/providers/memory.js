...
const { Dirent, stringToFlags } = require('internal/fs/utils');
...
class MemoryProvider extends VirtualProvider {
  ...
  openSync(path, flags, mode) {
    ...
    // Validate and interpret the flags the way fs does. Comparing the string
    // form directly misses the aliases ('as', 'rs', ...) and accepts values
    // fs rejects.
    const openFlags = stringToFlags(flags);

    const isCreate = (openFlags & O_CREAT) !== 0;
    // O_EXCL only has an effect together with O_CREAT.
    const isExclusive = isCreate && (openFlags & O_EXCL) !== 0;
    const isWritable = (openFlags & (O_WRONLY | O_RDWR)) !== 0;
    // O_TRUNC changes the file without asking for a writable descriptor, so
    // the read-only check needs more than isWritable. O_CREAT only writes
    // when the entry turns out to be missing, which is checked below.
    if (this.readonly && (isWritable || (openFlags & O_TRUNC) !== 0)) {
    ...
    return new MemoryFileHandle(normalized, flags, mode ?? entry.mode, entry.content, entry, getStats, openFlags);
  }
