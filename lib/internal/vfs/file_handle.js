...
class MemoryFileHandle extends VirtualFileHandle {
  ...
  /**
   * @param {string} path The file path
   * @param {string|number} flags The open flags as the caller passed them
   * @param {number} [mode] The file mode
   ...
   * @param {number} [openFlags] The open flags in numeric form, which is what
   *   decides the behaviour. Derived from `flags` when not given.
   */
  constructor(path, flags, mode, content, entry, getStats, openFlags) {
    super(path, flags, mode);
    this.#content = content;
    this.#size = content.length;
    this.#entry = entry;
    this.#getStats = getStats;
    this.#openFlags = openFlags ?? stringToFlags(flags);

    // Handle different open modes
    if ((this.#openFlags & O_TRUNC) !== 0) {
      // Write mode: truncate
    ...
    } else if ((this.#openFlags & O_APPEND) !== 0) {
   ...
   */
  #checkWritable() {
    if ((this.#openFlags & (O_WRONLY | O_RDWR)) === 0) {
   ...
   */
  #checkReadable() {
    // O_RDONLY is 0 and O_RDWR does not carry the O_WRONLY bit, so the bit
    // being set means write-only.
    if ((this.#openFlags & O_WRONLY) !== 0) {
   ...
   */
  #isAppend() {
    return (this.#openFlags & O_APPEND) !== 0;
  }
